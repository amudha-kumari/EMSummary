...
The summary generation is based on an NLP model ( T5 or other summarization models), and the resulting summaries
are saved to a specified output file.

Usage:
    python generate_summaries_with_T5.py --in_file /path/to/metadata_file --out_file out_file.tsv
...
# Function to summarize using T5 (Hugging Face pipeline)
def t5_summary(text):
    cleaned_text = ' '.join(text.split())
    summarizer = pipeline("summarization", model="google/flan-t5-large")  # or "t5-large"
...
def generate_summaries(metadata):
    ...
    for index, row in metadata.iterrows():
        ...
        if len(full_text.split()) < 50:  # Arbitrary: if fewer than 50 words
            ...
            summary = t5_summary(full_text)

        summaries.append({
            "EMDB_ID": row["EMDB_ID"],
            "EM_Method": row["EM_Method"],
            "NLP_Method": "T5" if len(full_text.split()) >= 50 else "TextRank",
            "Summary": summary
        })
