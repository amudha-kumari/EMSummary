...
def biobart_summary(text):
    cleaned_text = ' '.join(text.split())

    summarizer = pipeline(
        "summarization",
        model="andkelly21/t5-small-finetuned-pubmed",
        tokenizer="andkelly21/t5-small-finetuned-pubmed"
    )
...
def generate_summaries(metadata):
    ...
    for index, row in metadata.iterrows():
        ...
        if len(full_text.split()) < 50:
            ...
            summary = biobart_summary(full_text)
            method_used = "t5-small-finetuned-pubmed"
